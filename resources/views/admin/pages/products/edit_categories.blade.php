...
@section('content')
  ...
  <form action="/admin/action/edit_categories" method="POST">
    {{ csrf_field() }}
    ...
    <div class="mb-3">
      <label for="InputName" class="form-label">Выберите категорию для изменения:</label>
      <div class="mb-3">
        <select id="selectCategoriesName" onblur='this.size=1;' onfocus='this.size=getSizeSelect();'
          ...
          <option value="-1" selected>Добавить категорию</option>
          {{$i=0}}
    ...
    <div class="mb-3">
      ...
      <input type="text" name="categoriesName" placeholder = 'Укажите название категории' class="form-control" id="InputNameCategories" aria-describedby="nameHelp">
      <div id="nameHelp" class="form-text">Название категории (будет доступно в в выборе где находится товар)</div>
    </div>
